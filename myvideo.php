...
print  "<h1 class='h1'>".$username."さんの投稿動画</h1>";
?>
  <div class="myvideos">
    <?php
$mysql = new PDO("mysql:host=localhost;dbname=hew","root","");
$sql = "SELECT * FROM video LEFT OUTER JOIN account ON account.id = video.userid where account.id= :id order by video.videoid DESC";
$stmt = $mysql->prepare($sql);
